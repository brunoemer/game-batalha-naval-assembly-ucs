...
string db 'Timer tick tocking...','$'
	...
	tableTitleRes db 	179, "   Resultado", 9 dup(' '), 179
...
DELAY proc ; delay de 1 segundo
	...
	push cx
	push dx
	;mov cx, 100 ; tempo de espera em milisegundos
	;mov dx, 10000 ; tempo de espera em milisegundos
	;mov ah, 86h ; servico wait do bios
	;int 15h ; erro funcao nao suportada
...
TIRO_RECEBE proc
	...
	; pegar depois a coordenada da ppa na porta b
	mov dx, 1632h ; posicao 22 x 50
	call MOV_CURSOR
	call LE_CHAR
	sub al, '0' ; transforma em numero
	...
	mul ah ; deslocamento x2
	add al, 27 ; base
	; remover ate aqui e pegar da ppa
	...
	jmp TIROR_CONTINUA
	TIROR_ACERTOU: inc resAdvAcertos
	TIROR_CONTINUA: mov bl, 4
	; fazer contagem dos afundados
	call ESC_CHAR
...
; fazer:
; refazer proc PRINT_CONTENT_MATRIZ sem usar matriz da memoria
; fazer bordas da caixo inferior

; duvidas:
; como simular para testar outro jogador? abrir 2 programas para conversar com a ppa por qual porta paralela?
; como usar timer com a int 1ch
; melhor maneira de usar contador global?
